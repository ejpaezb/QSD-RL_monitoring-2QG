...
class IonTrap(object):
    """IonTrap class."""
    def __init__(self, N, ion_i, ion_j, omegas_path, g_ki_path, steps, nbar_c, time, extended_mu=False):
    ...
    def selectOptimizedPulse(self, stp, tau, mu, threshold=0.999, best=False, verbose=False):
        self.steps = stp
        ...
        for i in range(self.steps):
            ...
            if (fidelity > threshold):                
                f_list.append([i, max_rabi, np.real(fidelity), pulse])
                if verbose:
                    print('---- ---- ---- ----')
                    print('M {}'.format(max_rabi), 'f {:1.20f}'.format(np.real(fidelity)), 'alpha {}'.format(np.sum([np.abs(self.alpha(self.ion_i, k, tau, mu)) for k in range(self.N)])), 'chi {}'.format(self.chi(self.ion_i, self.ion_j, tau, mu)), np.reshape(pulse, (-1,)))
                    print('---- ---- ---- ----')

        
        if best:
            # sort the results based on the fidelities
            f_list.sort(key=lambda x: float(x[2]), reverse=True)
        else:
        ...
        return _bestF, _optPulse, _maxRabi, _scaleFactor, f_list

    def generatePulse(self, time, n_segments, best=False, verbose=False):
        # Multi-segment pulse
        f_list = []
        f_rabi = []
        pulse_list = []
...
#             _bestF, _optPulse, _maxRabi = self.selectOptimizedPulse(n_segments, time, muTmp, best, verbose)
            _bestF, _optPulse, _maxRabi, _scale = self.selectOptimizedPulse(n_segments, time, muTmp, best, verbose)
