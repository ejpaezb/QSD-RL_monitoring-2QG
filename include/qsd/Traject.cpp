...
//
// Same as sumExp, but stores information in an STD::vector.
//
//
...
//
    {
		...
		while (k < nTrajectory) {
            ...
            auto end = std::chrono::high_resolution_clock::now();
//            std::cout<< "total time:" << std::chrono::duration_cast<std::chrono::seconds>(end - begin).count()<<std::endl;
...
//
    {
        ...
        for (nstp = 1; nstp <= MAXSTP; nstp++) {
            if ((t + h - t2) * (t + h - t1) > 0.0) h = t2 - t;
            if (rndm != 0) {                // any noise? just white noise...
                stochasticFlag = 1;
                // set flag for derivs to generate stoch. part and store in newsum
                for (int i = 0; i < nL; i++)            // for each Lindblad...
                    dxi[i] = (*rndm)();            // ...independent 'white' noise
            }
            newsum = 0;
            rkqs(t, h, eps, hdid, hnext);            // deterministic part
            if (hdid == h) ++nok; else ++nbad;
...
/*
 See Eduardo Paez thesis Chapter 3.
1. Computes the change of the state psi in a timestep dt due to the
deterministic part of the QSD equation.
2. When stochasticFlag is set, computes the sum for the stochastic part per Lindbladian.
*/

    {
		std::complex<double> expect;
        double sum = 0.0;
		std::complex<double> stochSum = 0.0;
        ...
        for (int count = 0; count < nL; count++) {
            ...
            temp1 *= L[count](t);
            expect = psi * temp1;
            if (stochasticFlag != 0) {  //Stochastic part of QSD?
                temp0 = temp1;
                temp0 *= dxi[count];
                newsum += temp0;
            ...
            temp1 *= Ldag[count](t); //Ldag*L: can be removed from this loop to H
            temp1 *= -0.5;
            dpsi += temp1;
        }
        sum *= 0.5;
