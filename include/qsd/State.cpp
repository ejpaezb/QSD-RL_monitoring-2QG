...
//  IM/O operators for enum FreedomType; necessary of IM/O of States
...
// might cause errors.
    ...
    {                        // ground state
#ifndef OPTIMIZE_QSD
        if (n < 2) error("Invalid dimension size in State constructor no!");
    ...
    {                            // Fock state
#ifndef OPTIMIZE_QSD
        if (n < 2) error("Invalid dimension size in State constructor L:360!");
        if (form != FIELD)
    ...
    {                             // coherent state
#ifndef OPTIMIZE_QSD
        if (n < 2) error("Invalid dimension size in State constructor 2!");
        if (form != FIELD)
            error("Constructor only valid for FIELD type.");
#endif
        std::vector<std::complex<double>>().swap(data);
...
// Constructor for excited coherent state
    {
#ifndef OPTIMIZE_QSD
        if (n < 2) error("Invalid dimension size in State constructor 3!");
        if (form != FIELD)
            error("Constructor only valid for FIELD type.");
#endif
		std::vector<std::complex<double>>().swap(data);
        fock(n, nstate);    // call Fock state function
        coord = alpha;    // displace in shaving_phase space
    }
...
//
// Reset the value of the shaving_phase space coordinate for a freedom.
    {
#ifndef OPTIMIZE_QSD
        if ((theFreedom < 0) || (theFreedom >= nFreedoms))
...
// Move the center of coordinates to a new position in shaving_phase space.
...
// Recenter the coordinates at the current mean position in shaving_phase space.
    ...
    void State::recenter(int theFreedom, double shiftAccuracy) {
        ...
        alpha = -((*this) * dummy) / magnitude;    // get relative coords in shaving_phase space
    ...
    void State::moveCoords(const std::complex<double> &alpha, int theFreedom, double shiftAccuracy) {
        ...
        if (nFreedoms == 1) {                // 1 freedom case
            ...
            for (int i = 0; i < numshifts; i++) {
                ...
                phase += imag(delta * conj(theCoord));    // sum up shaving_phase changes
                theCoord -= delta;
            }
            coord = theCoord;
        } else {                    // multi-freedom case
        ...
        phase *= std::complex<double>(0, 1); // shaving_phase.timesI();
        (*this) *= exp(phase);            // remove shaving_phase change
